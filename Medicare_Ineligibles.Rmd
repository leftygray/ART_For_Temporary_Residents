...
saveFileTag <- " Test"  # Tag for output file names to specify scenarios
...
if (!loadVars || !loadResults){
  ...
  stochasticSims <- 1 # Number of sims for each parameter set if stochastic calculations are set-up
  ...
  oldsaveFileTag <- saveFileTag; oldsaveVars <- saveVars; oldsaveResults <- saveResults;
  ...
  loadFileTag <- oldvloadFileTag; loadVars <- oldloadVars; loadResults <- oldloadResults; useSeed <- olduseSeed
  saveFileTag <- oldsaveFileTag; saveVars <- oldsaveVars; saveResults <- oldsaveResults;
}
```
...
Our analysis includes an estimate of the annual cost of providing ART, care and support to Medicare ineligibles and their partners who become infected. We obtained estimates of the costs of providing treatment and care to Medicare ineligibles using previous work for Australian settings [@schneider2014cost]. For sexual partners of Medicare ineligibles we estimate the 'lifetime' cost of providing care and treatment using estimates from the the UNited States [@schackman2006lifetime] (which were the only estimates available). By summing all costs associated with providing ART to Medicare ineligibles and the average lifetime cost of providing care and support to PLHIV for their infected partners, we estimate the cost per infection averted. 
...
**Table 1** - Calculation input parameter ranges. Justifications for these parameter ranges are provided in the endnote
justifications. Samples are taken from these ranges assumning a uniform distribution. 
...
$p_g$ | Proprtion of population who are GBM | Given by the value $1-p_f-p_m$ | 1  
**Clinical parameters** | | | 
...
$$\beta'_x = a_x (1-p^c_x\epsilon_x)\beta_x.$$ For $x = f$, $m$ the number of acts and condom use are equal and given by the index $h$. Similarly we can incorporate the prevention effects of treatment and suppressed virus, to give the probability of transmission to a HIV-negative sexua partner $$\beta"_x = p_x[(1-\theta)\beta'_x + \theta(1-\psi)\beta'_x + \theta\psi(1-\epsilon_{ART})\beta'_x]$$ 
...
$$\left[\sum_{y = 1}^{years} (N(y)(c_{care} + \theta c_{ART})\right]$$
and the future costs of providing care and tretament to newly infected partners of Medicare ineligibles is
$$\Sigma c_{life}.$$
...
```{r calculations, echo=FALSE, messages = FALSE}
if (!loadResults){
...
costs <- melt(newinfections,id.vars=c("year","simulation"),measure.vars=c("costatras","lifecostbase","lifecostatras"),variable.name="scenario",value.name="totalcost")
costsSum <- aggregate(totalcost ~ simulation + scenario, data=costs, FUN=sum)

costplot <- ggplot(data = costsSum,aes(x=scenario,y=totalcost)) + theme_bw() + geom_bar(stat="identity")
print(costplot)
...
if (savePlots){dev.print(pdf,file=paste(outputFolder, "/figures/", currTime, " Results_Sims", fileTag,".pdf",sep =""))}
if (showPlots || savePlots){windows(width=12,height=4.5); print(grid.arrange(totalplot,avertplot,ncol = 2))}
if (savePlots){dev.print(pdf,file=paste(outputFolder, "/figures/", currTime, " Results_Total", fileTag,".pdf",sep =""))}
if (showPlots || savePlots){windows(width=6,height=4.5); print(costplot)}
if (savePlots){dev.print(pdf,file=paste(outputFolder, "/figures/", currTime, " Results_Costs", fileTag,".pdf",sep =""))}


```

```{r insertyplots1, echo=FALSE, messages = FALSE, fig.width=4, fig.height=3}
if(knitplots){print(simsplot)}
```

```{r insertyplots2, echo=FALSE, messages = FALSE, fig.width=6, fig.height=3}
