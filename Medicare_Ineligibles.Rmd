...
loadTag <- "" # Set to empty to run things a fresh. Manually enter file name tag to regenerate old results
loadResults <- FALSE # Set to FALSE to run things a fresh. If TRUE a corresponding input file needs to exist
saveVars <- FALSE
...
if (nchar(loadtag) == 0){
  ...
  startyear <- 2012
  datayears <- 3 # Number of years we have data
  ...
  stochasticSims <- 10 # Number of sims for each parameter set if stochastic calculations are set-up
  ...
  load(paste(loadTag,' Inputs.R',sep="")
...
# If not loading from previously created file specifiy inputs
if (nchar(varTag) == 0 && nchar(resultsTag) == 0){
  ...
  prop_vs_base <- c(0.7,0.91,0.96) # Hard coded
  ...
  if (saveVars || saveResults){
    save(list = ls(all = TRUE), file = paste(outputFolder, "/", currTime, fileTag, " Inputs.R", sep =""))
  }
}
```
