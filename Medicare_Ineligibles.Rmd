...
if (!loadVars || !loadResults){
  ...
  runyears <- 6        # number of years to run the model. Short term 5 to
                       # 10
  numsims <- 20      # 1000 number of sampled parameter sets to run
  stochastic <- TRUE   # run stochastic equations?
  stochasticSims <- 5 # 20 number of sims for each parameter set if
...
if (!loadVars && !loadResults){
  ...
  artcost <- c(7000, 15000) # for Medicare ineligibles. Assumed everyone 
                            # is on first line ART
...
ART over the period of analysis. From Schneider et. al., the average 
annual cost of first-line drugs is \$10,685 (\$6,945- \$14,424) 
[@schneider2014cost]. Using this value, we assume a range in the annual
ART cost of \$`r format.data(artcost[1], places = 0)` to \$`r format.data(artcost[2], places = 0)`.
...
class and summing produces the undiscounted cost presented here. To
...
#                 variable.name = "scenario", value.name = 
# "newinfections") 
# 
...
infectsSum <- newinfections %>%
  ...
  gather("scenario", "newinfections", 3:4) %>%
  group_by(scenario, simulation) %>% 
  summarise(newinfections = sum(newinfections)) %>%
...
avertedSum <- newinfections %>%
  ...
  gather("scenario", "totalaverted", 3) %>%
  group_by(scenario, simulation) %>% 
...
# Costs
medacost <- median(filter(costsSum, scenario == "costatras")$totalcost)
iqracost <- IQR(filter(costsSum, scenario == "costatras")$totalcost)

medacostdisc <- median(filter(costsSum, 
                              scenario == "costatrasdisc")$totalcost)
iqracostdisc <- IQR(filter(costsSum, 
                              scenario == "costatrasdisc")$totalcost)
