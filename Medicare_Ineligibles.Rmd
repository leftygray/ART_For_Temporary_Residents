...
loadFileTag <- "output/15-02-21 15-20-37 v2.0 Draft" # Set to empty to run things a fresh. Manually enter file name tag to regenerate old results. 
...
if (!loadVars || !loadResults){
  ...
  stochasticSims <- 2 # Number of sims for each parameter set if stochastic calculations are set-up
...
Figure 2 shows the distributions for the cumulative number of new infections in partners of Medicare ineligibles for the baseline scenario and if ART is provided to all Medicare ineligibles. Providing treatment to all Medicare ineligibles will avert a median of `r avertmed`  new infections (IQR: `r avertmed-avertiqr/2` - `r avertmed+avertiqr/2`) over `r runyears` years.

**Figure 2** - Total number of number of new infections (left) and the distribution in infections averted (right) over `r runyears` years after all HIV-positive people ineligible for Medicare are provided ART.
...
Figure 3 shows the cumulative costs for providing ART to Medicare ineligible people for the next `r runyears` years and the resulting lifetime costs of providing treatment to the people they infected during this period. The median undiscounted cumulative cost when ART is expanded to all Medicare ineligible PLHIV is \$`r formatC(round(medtotcost,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(medtotcost -iqrtotcost/2,4),format="d",big.mark=',')` - \$`r formatC(round(medtotcost + iqrtotcost/2,4),format="d",big.mark=',')`) (taking the median of the sum for each simulation). This cost is lower than the cost of providing treatment to newly infected people when ART access is denied to PLHIV who are Medicare ineligible (median \$`r formatC(round(medlbcost,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(medlbcost -iqrlbcost/2,4),format="d",big.mark=',')` - \$`r formatC(round(medlbcost + iqrlbcost/2,4),format="d",big.mark=',')`); Figure 3). However, when discounting is taken into count, the long term costs reduce substantially to \$`r formatC(round(medtotcostdisc,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(medtotcostdisc -iqrtotcostdisc/2,4),format="d",big.mark=',')` - \$`r formatC(round(medtotcostdisc + iqrtotcostdisc/2,4),format="d",big.mark=',')`) and the total costs for the expanded access scenario are slightly higher than the denied access scenario median \$`r formatC(round(medtotcostbasedisc,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(medtotcostbasedisc -iqrtotcostbasedisc/2,4),format="d",big.mark=',')` - \$`r formatC(round(medtotcostbasedisc + iqrtotcostbasedisc/2,4),format="d",big.mark=',')`).

**Figure 3** - Median total costs for providing all Medicare ineligbles with ART and the lifetime treatment costs for partners of Medicare Ineligibles who acquire infection over `r runyears` years. The bars show the interquartile range in total costs across all simulations.
