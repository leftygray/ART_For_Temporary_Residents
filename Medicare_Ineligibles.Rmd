...
loadFileTag <-  "" 
# "output/15-03-13 13-15-17 v2.1 Draft" ATRAS report and submitted paper
...
if (!loadVars && !loadResults){
  ...
  lifecostYearly <- rep(costFourth, totalLifeYears) + monitoringCost
  lifecostYearly[1:yearsFirst] <- costFirst + monitoringCost
  lifecostYearly[(yearsFirst + 1):(yearsFirst + yearsSecond)] <-
    costSecond + monitoringCost
  lifecostYearly[(yearsFirst + yearsSecond + 1):(yearsFirst + yearsSecond 
    + yearsThird)] <- costThird + monitoringCost
...
**Treatment costs** | | |  
$c_{ART}$ | Average annual undiscounted cost of providing ART to Medicare ineligibles | [`r FormatData(artcost[1], places = 0, money = TRUE)` - `r FormatData(artcost[2], places = 0, money = TRUE)`] | 5   
$c_{life}$ | Average undiscounted lifetime cost of providing ART post infection | [`r FormatData(lifecostRange[1], money = TRUE)` - `r FormatData(lifecostRange[2], money = TRUE)`] | 6   
...
ART cost of \$`r FormatData(artcost[1], places = 0)` to \$`r FormatData(artcost[2], places = 0)`.
...
colnames(summaryResults) <- c("Indicator", 
                              "Status-quo scenario \n (median, IQR)", 
                              "Expanded access scenario \n (median, IQR)")

```
 
**Table 1** - Summary results for status-quo scenario and the expanded 
access scenario. The results show the median and inter-quartile range (IQR)
of all simulations for each scenario. NA = not applicable.

```{r inserttable1, echo=FALSE, messages = FALSE, warning = FALSE}
