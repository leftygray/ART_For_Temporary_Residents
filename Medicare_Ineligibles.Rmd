...
**HIV transmission parameters** | | |   
$\beta_n$ | Annual transmission probability from non-MSM to partners in the absence of ART  |  [`r round(betanRange[1],4)` - `r round(betanRange[2],4)`] | 3    
$\beta_m$ | Annual transmission probability from MSM to partners in the absence of ART |  [`r round(betamRange[1],4)` - `r round(betanRange[2],4)`] |  3  
...
Figure 3 shows the cumulative costs for providing ART to Medicare ineligible people for the next 5 years and the resulting lifetime costs of providing treatment to the people they infected during this period. The median undiscounted cumulative cost when ART is expanded to all Medicare ineligible PLHIV is \$`r formatC(round(mednetcost,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(mednetcost -iqrnetcost/2,4),format="d",big.mark=',')` - \$`r formatC(round(mednetcost + iqrnetcost/2,4),format="d",big.mark=',')`) (taking the median of this sum for each simulation). This cost is lower than the cost of providing treatment to newly infected people when ART is not provided to PLHIV who are Medicare ineligible (Figure 3). However, when discounting is taken into count, the long term costs reduce substantially to \$`r formatC(round(mednetcostdisc,4),format="d",big.mark=',')` (IQR: \$`r formatC(round(mednetcostdisc -iqrnetcostdisc/2,4),format="d",big.mark=',')` - \$`r formatC(round(mednetcostdisc + iqrnetcostdisc/2,4),format="d",big.mark=',')`) but the total costs for the expanded access scenario end up being slightly greater than the baseline scenario of no ART provision to Medicare ineligibles.  
...
$$\sum_{y = 1}^{years} N(y) \theta c_{ART}/(1+r_disc)^y$$ (undiscounted costs are calculated by setting the discount rate to $1$). 
The total future costs of providing treatment to newly infected partners of Medicare ineligibles is given by 
$$\sum_{y = 1}^{years I(y)*\sum_{l = 1}^{y_{art}} c_{ART}(l)/(1+r_disc)^{y+t_i$$
where $y_{ART}$ is the number of years infected people will be on ART, $c_{ART}(l)$ is the annual cost of ART $l$ years from initiation, and $t_i$ is the time between infection initiating ART (undiscounted costs are calculated by setting the discount rate to $0$).  
