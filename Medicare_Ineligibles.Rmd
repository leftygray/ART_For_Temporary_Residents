...
if (!loadVars && !loadResults){
  ...
  lifecostYearly <- rep(1,totalLifeYears)
...
if (!loadResults){
  ...
  discountAtrasVec <- melt(t(discountAtras)); discountAtrasVec <- discountAtrasVec[c(2,1,3)]; colnames(discountAtrasVec) <- c("simulation","year","costatrasDiscounted")
