...
# Convert to matrices. Population size calulated by combining the initial population size and growth rate.
popsizeMatrix <- matrix(0,numsims,runyears)
for (ii in 1:numsims){
  popsizeMatrix[ii,] <- runif(1,popsize[1],popsize[2]) * runif(1,pop_growth[1],pop_growth[2])^c(1:runyears)
}

prop_fMatrix <- matrix(runif(numsims,prop_f[1],prop_f[3]),numsims,runyears)
prop_mMatrix <- matrix(runif(numsims,prop_m[1],prop_m[3]),numsims,runyears)
...
# Convert to matrices. This is done by first converting proportions on ART and with viral suppression to vectors of equal length 
# to number of simulation years and then inverting twice so the uncertianty factor does not produce a value great than 1. If the
# number of simulations years is greater than the number of data values the final data value is assumed to remain constant.
...
prop_art_baseMatrix <- matrix(0,numsims,runyears)
prop_vs_baseMatrix <- matrix(0,numsims,runyears)
for (ii in 1:numsims) {
  ...
  prop_art_baseMatrix[ii,] <- 1-tempFactor_art * (1-prop_art_baseVec)
  prop_vs_baseMatrix[ii,] <- 1-tempFactor_vs * (1-prop_vs_baseVec)
}
...
# Convert to matrices
heteroactsMatrix <- matrix(runif(numsims,heteroacts[1],heteroacts[3]),numsims,runyears)
heterocondomMatrix <- matrix(runif(numsims,heterocondom[1],heterocondom[3]),numsims,runyears)
gbmactsMatrix <- matrix(runif(numsims,gbmacts[1],gbmacts[3]),numsims,runyears)
gbmcondomMatrix <- matrix(runif(numsims,gbmcondom[1],gbmcondom[3]),numsims,runyears)
...
# Convert to matrices
betafMatrix <- matrix(runif(numsims,betaf[1],betaf[3]),numsims,runyears)
betamMatrix <- matrix(runif(numsims,betam[1],betam[3]),numsims,runyears)
betagMatrix <- matrix(runif(numsims,betag[1],betag[3]),numsims,runyears)

epsilon_condomMatrix <- matrix(runif(numsims,epsilon_condom[1],epsilon_condom[3]),numsims,runyears)
epsilon_artMatrix <- matrix(runif(numsims,epsilon_art[1],epsilon_art[3]),numsims,runyears)
...
infectsm <- matrix(0,numsims,runyears)
infectsf <- matrix(0,numsims,runyears)
infectsg <- matrix(0,numsims,runyears)

# Do the calculations
for (ii in 1:numsims){
  # Calculate population beta values which are the same for both ATRAS data and counterfactual scenario. MAY HAVE TO CHANGE THIS TO A
  # STOCHASTIC FORM IF THE INCIDENCE IS REALLY SMALL. Can do this through sampling using rbinom().
  infectsm[ii,] <- popinfects(popsizeMatrix[ii,]*prop_mMatrix[ii,],infect_prob(heteroactsMatrix[ii,],heterocondomMatrix[ii,],
                  epsilon_condomMatrix[ii,],prop_art_baseMatrix[ii,],prop_vs_baseMatrix[ii,],epsilon_artMatrix[ii,],
                  betafMatrix[ii,]),TRUE)
    
    
#    heteroactsMatrix[ii,]*(1-prop_art_baseMatrix[ii,]*prop_vs_baseMatrix[ii,]*
#                   epsilon_artMatrix[ii,])*(1-heterocondomMatrix[ii,]*epsilon_condomMatrix[ii,]) * betamMatrix[ii,]

  infectsf[ii,] <- popsizeMatrix[ii,]*prop_fMatrix[ii,]*heteroactsMatrix[ii,]*(1-prop_art_baseMatrix[ii,]*prop_vs_baseMatrix[ii,]*
                   epsilon_artMatrix[ii,])*(1-heterocondomMatrix[ii,]*epsilon_condomMatrix[ii,]) * betafMatrix[ii,]
#  infectsg[ii,] <- popsizeMatrix[ii,]*(1-prop_mMatrix[ii,]-prop_mMatrix[ii,])*homoactsMatrix[ii,]*(1-prop_art_baseMatrix[ii,]*
#                   prop_vs_baseMatrix[ii,]*epsilon_artMatrix[ii,])*(1-homocondomMatrix[ii,]*epsilon_condomMatrix[ii,]) * 
#                   betagMatrix[ii,] 
}
