...
# Convert to matrices
popsizeMatrix <- matrix(runif(numsims,popsize[1],popsize[3]),numsims,runyears)
pop_growthMatrix <- matrix(runif(numsims,pop_growth[1],pop_growth[3]),numsims,runyears)
...
for (ii in 1:numsims) {
  ...
  prop_vs_factorMatrix[ii,] <- 1-tempFactor_vs * (1-prop_vs_factorVec)
}
...
$$I_x = (1-\theta)\beta'_x + \theta(1-\psi)\beta'_x + \theta\psi(1-\epsilon_{ART})\beta'_x$$ 
...
$$I(y) = [a_h(1-p^c_h\epsilon_c)(\beta_f + \beta_m) + a_g(1-p^c_g\epsilon_c)\beta_g](1-\theta(y)\psi(y)\epsilon_{ART}).$$ 
...
# R code to run the simulations and actually does the calculations
```
